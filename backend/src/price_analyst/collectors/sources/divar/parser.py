"""Deterministic Divar listing and detail parser."""
...
from price_analyst.collectors.sources.divar.selectors import LISTING_LINK_SELECTOR, PRICE_SELECTORS
...
class DivarParser:
    def __init__(self, *, base_url: str = "https://divar.ir") -> None:
    ...
    def parse_product_html(
        ...
        *,
        ...
        source_id = self._listing_id(url) or as_text(product.get("sku")) or "unknown"
        ...
        return [
            self._make_offer(
                ...
                availability=parse_availability(as_text((offer or {}).get("availability")) or text),
    ...
    def _dom_candidates(self, soup: BeautifulSoup) -> list[SearchCandidate]:
        ...
        for anchor in soup.select(LISTING_LINK_SELECTOR):
            ...
            source_id = self._listing_id(url)
    ...
    def _candidate_from_schema(self, item: dict[str, Any]) -> SearchCandidate | None:
        ...
        source_id = self._listing_id(url) or as_text(item.get("sku"))
        if not source_id:
            return None
        attributes = self._attributes(title)
