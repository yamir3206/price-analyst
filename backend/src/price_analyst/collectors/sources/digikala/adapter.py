"""Async Digikala adapter using public search/product HTML."""
...
class DigikalaAdapter:
    ...
    def __init__(
        ...
        *,
        ...
        parser: DigikalaParser | None = None,
        retry_policy: RetryPolicy | None = None,
    ) -> None:
        self._client = client
        self._base_url = base_url.rstrip("/")
        self._parser = parser or DigikalaParser(base_url=self._base_url)
    ...
    async def fetch_details(
        ...
        if urlsplit(candidate.url).netloc.lower() != urlsplit(self._base_url).netloc.lower():
            raise ValueError("refusing to fetch a product URL outside the configured Digikala host")
        response = await get_with_retry(
            self._client,
            candidate.url,
            timeout=context.timeout_seconds,
