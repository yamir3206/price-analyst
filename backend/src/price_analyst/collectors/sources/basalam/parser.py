"""Deterministic Basalam JSON-LD and DOM parser."""
...
from price_analyst.collectors.sources.basalam.selectors import (
    PRICE_SELECTORS,
...
class BasalamParser:
    def __init__(self, *, base_url: str = "https://basalam.com") -> None:
    ...
    def parse_product_html(
        ...
        *,
        ...
        if price is None:
            price = self._price_from_card(soup, default_currency=Currency.UNKNOWN)
        if price is None:
            return []
        card_text = soup.get_text(" ", strip=True)
        seller = schema_seller(offer) or self._seller_from_card(soup)
        return [
            self._make_offer(
                offer_id=f"basalam:{source_id}",
                ...
                availability=parse_availability(
                    as_text((offer or {}).get("availability")) or card_text
                ),
                condition=parse_condition(
                    as_text((offer or {}).get("itemCondition")) or card_text
                ),
        ...
        for anchor in soup.select(PRODUCT_LINK_SELECTOR):
            ...
            result.append(
                SearchCandidate(
                    ...
                    seller=self._seller_from_card(card or anchor),
    ...
    def _schema_offer(self, offers: Any) -> dict[str, Any] | None:
    ...
    def _price_from_card(
        ...
        *,
        ...
        for selector in PRICE_SELECTORS:
            for element in card.select(selector):
                for value in (
                    ...
                    parsed = parse_numeric_money(value, default_currency) if value else None
                    if parsed:
                        return parsed
        ...
        return parse_labeled_money(text, default_currency=default_currency) or (
            parse_last_numeric_money(
                text,
                default_currency=default_currency,
            )
    ...
    def _seller_from_card(self, card: Tag) -> str | None:
